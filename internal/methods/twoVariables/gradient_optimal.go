...
// Алгоритм:
//
//	На каждой итерации вычисляется градиент g = ∇f(xk).
//	Если ‖g‖ < eps — остановка.
//	Находим оптимальный шаг α*: минимизируем φ(α) = f(xk − α·g) по α ∈ [0, αmax]
//	методом золотого сечения (повторное использование логики из Lab 1).
//	x_{k+1} = xk − α* · g
